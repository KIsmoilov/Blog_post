...
RSpec.describe 'posts#index', type: :feature do
  describe 'Post spec' do
    before(:each) do
      @user = User.create(name: 'Khusniddin', photo: 'Khusniddin.png', bio: 'Lorem.', posts_counter: 0)
    ...
    it "shows user's profile picture" do
      all('img').each do |i|
        expect(i[:src]).to eq('Khusniddin.png')
      end
    end

    it 'shows the users username' do
      expect(page).to have_content('Khusniddin')
    end
